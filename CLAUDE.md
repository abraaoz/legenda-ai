...
- **OpenSubtitles `User-Agent`**: precisa ser o **nome EXATO do consumer registrado** + versão (ex.: `legendaAIpramim v1.0.0`), senão a API responde **403 "User-Agent header is wrong"** na busca/download. Está em `USER_AGENT` (`opensubtitles.ts`).
...
- **Tradução idempotente e retomável** (`translate.ts`): grava o `.<lang>.ai.srt` **a cada lote** (salvamento incremental). Ao (re)iniciar, lê o parcial existente e **valida o alinhamento por timestamps** contra a fonte; se alinhar, retoma de `parsed.length`; senão recomeça. Completo → retorna sem trabalho (idempotente). Retorna `{savedPath, done, total}`.
