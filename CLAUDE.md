...
- **Tocar na TV (Chromecast / Google Cast)** — `src/bun/lib/cast/`, tudo em Bun nativo (sem lib Node). Fluxo: descobrir → conectar → servir mídia → LOAD com legenda VTT → controles. **Validado ponta-a-ponta num Chromecast real.** Botão "📺 Tocar na TV" no card (escolhe dispositivo + uma legenda `.srt` externa); barra de status/controle no topo (⏸/▶/⏹ + progresso). RPC: `castDiscover`/`castStart`/`castControl` + mensagem `castStatus`. **Só UMA sessão ativa** (o `manager` encerra a anterior). **Aprendizados que custaram caro (não regredir):**
  ...
  - **Codec/transcode (com seek, via HLS)**: o `manager` faz `ffprobe` do vídeo. Se for compatível (**H.264** + áudio comum estéreo), serve **cru com byte-range** (206, seek nativo). Se não (ex.: **HEVC/x265**, áudio multicanal 7.1 — comum em rip Blu-ray), transcoda via **HLS**: `server.ts` gera uma **playlist VOD `.m3u8`** (anuncia a duração inteira, do `ffprobe`) + **segmentos `.ts` de 6s transcodados SOB DEMANDA** (`ffmpeg -ss <t> -t 6 ... -output_ts_offset <t>`, decode+encode em HW `videotoolbox` — ~1s por segmento no Apple Silicon). **O SEEK funciona por QUALQUER controlador** (Google Home, TV, o próprio app), porque cada ponto da barra vira um pedido de segmento independente — validado no Chromecast real (seek pra frente e pra trás). LOAD com `contentType: application/x-mpegURL`, `streamType: BUFFERED`, `duration` do probe. Cada segmento é um `Bun.spawn` curto (rastreados num Set, mortos no stop). Fora do macOS cairia pro `libx264` (software) — não testado.
