...
- **Tocar na TV (Chromecast / Google Cast)** — `src/bun/lib/cast/`, tudo em Bun nativo (sem lib Node). Fluxo: descobrir → conectar → servir mídia → LOAD com legenda VTT → controles. **Validado ponta-a-ponta num Chromecast real.** Botão "📺 Tocar na TV" no card (escolhe dispositivo + uma legenda `.srt` externa); barra de status/controle no topo (⏸/▶/⏹ + progresso). RPC: `castDiscover`/`castStart`/`castControl` + mensagem `castStatus`. **Só UMA sessão ativa** (o `manager` encerra a anterior). **Aprendizados que custaram caro (não regredir):**
  ...
  - **Codec/transcode**: o `manager` faz `ffprobe` do vídeo. Se for compatível (**H.264** + áudio comum estéreo), serve **cru com byte-range** (206, permite seek). Se não (ex.: **HEVC/x265**, áudio multicanal 7.1 — comum em rip Blu-ray), **transcoda on-the-fly** pra H.264/AAC estéreo via ffmpeg com o **encoder de HW `h264_videotoolbox`** (realtime no Apple Silicon), servido como **fMP4 progressivo** (`frag_keyframe+empty_moov+default_base_moof`, um pipe do ffmpeg por request, morto no stop/nova request). **No modo transcode NÃO há seek** (é pipe ao vivo) e a duração vem do `ffprobe` (a do receiver é falsa/cresce — o `onStatus` prefere a do probe). Fora do macOS cairia pro `libx264` (software) — não implementado/testado ainda.
