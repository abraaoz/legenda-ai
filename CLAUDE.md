...
- **Tocar na TV** — `src/bun/lib/cast/`, tudo em Bun nativo (sem lib Node). Suporta **DOIS protocolos**, escolhidos por dispositivo (`CastDevice.protocol`): **Chromecast/Google Cast** (CASTV2) e **DLNA/UPnP** (Samsung Smart TV etc.). `castDiscover` roda os dois em paralelo (mDNS + SSDP) e mescla; `castStart` faz branch por `protocol`. Fluxo comum: descobrir → conectar → servir mídia (`server.ts`, compartilhado) → LOAD com legenda → controles. **Ambos validados ponta-a-ponta em hardware real** (Chromecast "Gordocast" e Samsung "The Frame 50"). Botão "📺 Tocar na TV" no card (dispositivo com ícone 📺 Cast / 📡 DLNA + legenda `.srt`); barra de status/controle no topo. RPC: `castDiscover`/`castStart`/`castControl` + mensagem `castStatus`. **Só UMA sessão ativa** (o `manager` encerra a anterior). O `CastStartArgs` leva `protocol` + `controlUrl` (DLNA). **Aprendizados que custaram caro (não regredir):**
  - **DLNA/Samsung (`dlna.ts` + SSDP em `discover.ts`)**: descoberta por **SSDP** (M-SEARCH multicast em 239.255.255.250:1900 procurando `MediaRenderer`; reusa `listLocalIPv4`/`lanCandidates`), busca o XML de descrição no `LOCATION` e extrai o `controlURL` do serviço **AVTransport**. Controle é **SOAP sobre HTTP** (`fetch` POST): `SetAVTransportURI`+`Play`/`Pause`/`Stop`/`Seek` (Unit `REL_TIME`, formato `H:MM:SS`). O receiver **não empurra status** → poll de `GetPositionInfo`+`GetTransportInfo` a cada 1s pra barra de tempo. Legenda: **SRT** (não VTT) via `sec:CaptionInfoEx` no DIDL-Lite + header `CaptionInfo.sec`. **A Samsung EXIGE `idleTimeout: 0` no `Bun.serve`** (o default de 10s aborta o stream de mídia → trava em BUFFERING). **A Samsung decodifica MUITO** (validado: HEVC/x265 + AAC 8ch tocam em **direct-play** com byte-range, seek/pause/legenda nativos via AVTransport).
  - **Decisão de transcode DLNA é EMPÍRICA, não por codec** (a pedido do usuário — nada de pré-transcodar o arquivo inteiro à toa): o `manager` (`startDlna`) **sempre tenta direct-play primeiro** (barato: serve o arquivo cru e dá LOAD) e `waitForPlayback` observa se a TV **realmente começa** (`PLAYING` + tempo avançando) por até **20s**. Só se ela **não** reproduzir é que converte. Margem de 20s de propósito: melhor esperar num fracasso raro do que converter um arquivo grande que só demorou a bufferizar. (O `needsTranscodeChromecast` por codec vale só pro **Chromecast**.)
  - **DLNA NÃO aceita transcode ao vivo** (custou horas de teste na TV real): tentado stream progressivo (MPEG-TS/MKV/MP4, com/sem `TimeSeekRange`, chunked) → a TV faz `HEAD`→`GET bytes=0-` e **imediatamente `GET bytes=<fim>-`** (sonda o índice/fim do arquivo) e, sem obtê-lo, **trava em IDLE/BUFFERING**. Arquivo crescente idem (o wait do fim nunca resolve). **Solução (só quando o direct-play falha): PRÉ-transcodar o vídeo INTEIRO** pra um `.mkv` temporário (`DlnaPreTranscode` em `server.ts`, progresso via `-progress pipe:1` → log) e servir por **byte-range** (aí toca e seeka perfeito). Lento, mas **raro**. O `manager` limpa o temp no `castStop` (e aborta se o usuário cancelar no meio). Um **arquivo transcodado completo** servido por byte-range tocou+seekou perfeito na The Frame; **stream ao vivo, não**.
  ...
  - **Alguns aparelhos IGNORAM a query mDNS multicast** (validado: projetor com Google TV, `md=C045RGN`) — ficavam **invisíveis** no app, enquanto o `dns-sd -B _googlecast._tcp` do macOS os via. Eles **respondem a query UNICAST DIRETA**, e não dá pra ouvir o multicast deles porque **o Bun não consegue bindar a 5353** (`EADDRINUSE`, nem com `reusePort` — o mDNSResponder segura a porta). **Solução: `sweepUnicast`** manda a query direta pra cada host do **/24 local** (~254 datagramas, rápido). Diagnóstico útil: `dns-sd -B/-L/-G` mostra o que o SO vê vs. o que o app vê.
  - **Filtro de aparelhos só de áudio**: no Cast, o TXT `ca` do mDNS é um bitmask e o **bit 0 (0x01) = saída de vídeo** (validado: Gordocast `ca=201221` e projetor `ca=264709` têm; Mi Smart Speaker `ca=198660` não). Se `ca` não vier, **mantém** (não filtra sem certeza). No DLNA, checa o `Sink` do `GetProtocolInfo` por algum `video/` (best-effort: erro/ausência → mantém).
  - **O Default Media Receiver do Cast NÃO toca HEVC — em NENHUM aparelho Cast** (validado no projetor com Google TV; não confundir "o hardware decodifica HEVC" com "o receiver aceita"): o `CC1AD845` é um **player web** de codecs limitados (H.264/VP8/VP9), e os apps do próprio Google TV (Netflix/YouTube) usam outro caminho. Matriz testada no projetor: H.264/MP4 ✅, **H.264/MKV ✅** (o container MKV NÃO é problema), HEVC/MKV ❌, HEVC/MP4 com áudio 8ch ❌ e com estéreo ❌ (o áudio também não era o problema), HEVC via HLS com `-c:v copy` ❌. **Conclusão: x265 SEMPRE transcoda no Cast**, e o `needsTranscodeChromecast` (por codec) já está certo — **não** vale trocar pelo fluxo empírico do DLNA (o HEVC sempre falharia e cairia no mesmo HLS, só mais devagar). É por isso que a **Samsung/DLNA dá qualidade original**: o DLNA entrega o arquivo pro **player nativo da TV**.
  - **Legenda**: o Cast só aceita **WebVTT** (o `server.ts` converte SRT→VTT: `WEBVTT\n\n` + vírgula→ponto nos tempos) e exige **CORS** (`Access-Control-Allow-Origin: *`) na resposta da legenda.
  - **Codec/transcode (com seek, via HLS estilo Jellyfin)**: o `manager` faz `ffprobe`. Se o vídeo for compatível (**H.264** + áudio comum estéreo), serve **cru com byte-range** (206, seek nativo). Se não (ex.: **HEVC/x265**, áudio 7.1 — rip Blu-ray), transcoda via **HLS** (`server.ts`, classe `HlsSession`): playlist **VOD `.m3u8`** com a duração inteira (do probe) + **UM ffmpeg contínuo** (`-f hls`, `h264_videotoolbox` decode+encode em HW, `-force_key_frames` a cada 6s, `-start_number`) que grava segmentos `.ts` sequenciais num dir temp. **Os segmentos são movidos pra RAM** (`Map<n, Uint8Array>`) e os arquivos apagados na hora — serve 100% da memória, o disco fica em ~1 arquivo (o que o ffmpeg escreve no momento). **No SEEK, o ffmpeg é REINICIADO no ponto** (`-ss segStart`) — eficiente na reprodução linear (sem re-seek por segmento) E seek funciona por QUALQUER controlador (Google Home, TV, o app). **Janela em RAM** dimensionada pelo setting **`castRamGb`** (default 0.5 GB; opções na aba Tradução das Configurações): a janela `behind`/`ahead` é derivada do budget (~40%/55% de `budget/6MB`), e há um **teto RÍGIDO em bytes** (`memBytes`, evicta os segmentos mais distantes da posição se passar do budget — sem leak). Seek dentro da janela é **instantâneo** (já em memória); mais longe reinicia (~1-2s). **Seek pela UI**: a barra de progresso do cast-bar é clicável (`castControl('seek', frac*duration)`). Validado no Chromecast real (linear + seek frente/trás; disco ~7 MB). LOAD com `contentType: application/x-mpegURL`, `duration` do probe. Gotchas (custaram caro): (1) **`Bun.$` fire-and-forget NÃO roda** — precisa `await`/`.then` senão o `rm` não executa; (2) **`killProc` num ffmpeg pausado (SIGSTOP) precisa de SIGCONT+SIGKILL** (SIGTERM é ignorado por processo parado → vazava no stop); (3) **`head` = maior segmento CONTÍGUO produzido** (na RAM ou em arquivo), não o scan do dir. Throttle (SIGSTOP/SIGCONT quando produz > `AHEAD` à frente) + evicção da RAM/arquivos fora da janela. Fora do macOS cairia pro `libx264` — não testado.
